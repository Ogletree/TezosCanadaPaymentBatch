...
namespace Tz.Net
{
    ...
    public class Rpc
    {
        ...
        public Rpc(string provider, string chain)
        {
            ...
            _provider = provider;
            _chain = chain;
            Log.Info($"Provider: {_provider}");
            Log.Info($"Chain: {_chain}");
        }

        public async Task<JObject> Describe()
        {
            // There is currently a weird situation in alpha where the RPC will not honor any request without a recurse=true arg. // 8 Aug 2018
        ...
        private async Task<List<OperationResult>> SendOperations(JToken operations, Keys keys, JObject head = null)
        {
            ...
            if (opResults.All(op => op.Succeeded))
            {
                ...
                var message = ((JValue)injectedOperation).Value.ToString();
                Singleton.Instance.LastOperation = message;
                Log.Info(message);
            }
            return opResults;
        }
        ...
        private async Task<JToken> ForgeOperations(JObject blockHead, JArray operations)
        {
            ...
            var forgeOperations = await QueryJ($"chains/{_chain}/blocks/head/helpers/forge/operations", contents);
            return forgeOperations;
        }
        ...
        public async Task<SendTransactionOperationResult> SendTransaction(Keys keys, string from, List<MyTrans> myTrans, BigFloat fee, BigFloat gasLimit = null, BigFloat storageLimit = null)
        {
            gasLimit = gasLimit ?? 10300;
            storageLimit = storageLimit ?? 0;
            ...
            foreach (var trans in myTrans)
            {
                var transaction = new JObject();
                operations.Add(transaction);
                transaction["destination"] = trans.Account;
                var value = trans.Reward.ToMicroTez().ToString(6);
                transaction["amount"] = new BigFloat(value).Round().ToString(); // Convert to microtez, truncate at 6 digits, round up
                transaction["storage_limit"] = storage;
                transaction["gas_limit"] = gas;
                //transaction["counter"] = (++counter).ToString();
                transaction["fee"] = fee.ToString();
